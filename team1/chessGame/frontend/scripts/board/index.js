...
chessboard.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("pieces")) {
    if (
      (isWhiteTurn && e.target.classList.contains("white-pieces")) ||
      (!isWhiteTurn && e.target.classList.contains("black-pieces"))
    ) {
    ...
    if (e.target.parentElement.classList.contains("valid-move")) {
      if (isWhiteTurn) {
        if (whiteInterval) {
          stopCountTime(whiteInterval);
        }
        blackInterval = countTimeBlack();
      } else {
        if (blackInterval) {
          stopCountTime(blackInterval);
        }
        whiteInterval = countTimeWhite();
      }
