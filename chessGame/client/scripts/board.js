    ...
    for (let row = 1; row < 9; row++) {
        for (let col = 1; col < 9; col++) {
            ...
            square.classList.add(
                "square",
                (row + col) % 2 === 0 ? "squareWhite" : "squareBlack"
            );
...
function renderTextBoardGame() {
    ...
    for (let row = 1; row < 9; row++) {
        ...
        row % 2 === 0
            ? (numberIndex.innerHTML += `<span class="numWhite">${counter--}</span>`)
    ...
    for (let col = 1; col < 9; col++) {
        ...
        if (col === 1) {
            textIndex.innerHTML += `<span class="textWhite">${
                bufferText[col - 1]
            }</span>`;
            continue;
        }
        ...
        col % 2 === 0
            ? (textIndex.innerHTML += `<span class="textBlack">${
                  bufferText[col - 1]
              }</span>`)
            : (textIndex.innerHTML += `<span class="textWhite">${
                  bufferText[col - 1]
